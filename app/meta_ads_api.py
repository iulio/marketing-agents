...
class MetaAdsAPI:
    ...
    def is_available(self) -> bool:
        return self.api is not None and META_AVAILABLE
    
    def create_campaign(self, campaign_data: Dict[str, Any]) -> Dict[str, Any]:
        if not self.is_available() or not self.ad_account_id:
    ...
    def get_campaign_metrics(self, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            account_id = self.ad_account_id if self.ad_account_id.startswith('act_') else f"act_{self.ad_account_id}"
            account = AdAccount(account_id)
            fields = ['campaign_id', 'campaign_name', 'campaign_status', 'impressions', 'clicks', 'spend', 'conversions', 'conversion_value', 'ctr', 'cpc', 'conversions_value_per_cost']
            params = {
                'level': 'campaign',
                'time_range': {
